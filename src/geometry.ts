...
function normalize(bounds: IBounds): IBounds[] {
    return [
        Geometry.Bounds.create(Geometry.Point.create(bounds.inner.r, 0), bounds.outer),
        Geometry.Bounds.create(bounds.inner, Geometry.Point.create(bounds.outer.r, TWO_PI))
    ]
}
...
export namespace Geometry {
    export namespace Point {
        ...
        export function translate(p: IPoint, delta_r: number, delta_theta: number): IPoint {
            return Geometry.Point.create(p.r + delta_r, p.theta + delta_theta);
        }
    }
        ...
        export function contains(bounds: IBounds, p: IPoint): boolean {
            if (isMalformed(bounds)) {
                return normalize(bounds)
                    .map(b => Geometry.Bounds.contains(b, p))
